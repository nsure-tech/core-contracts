...
contract Staking is Ownable, Pausable {
    ...
    struct Pool {
        IERC20   token;          // contract address, if it's an erc20 token
    ...
    constructor(address tokenAddr, uint256 pendingBlocks, uint256 minDepositNum) public 
    {
        ...
        pool.token          = IERC20(tokenAddr);
        pool.pendingBlocks  = pendingBlocks;
        pool.minDepositNum  = minDepositNum;
    }

    function getStakedBalance() external returns (uint256) {
        return pool.investors[_msgSender()].invests;
    }
    ...
    function getInvestorInfo(address _addr) external view
        returns(uint256 invest, uint256 withdrawal, uint256 pendingAtBlock)
    {
